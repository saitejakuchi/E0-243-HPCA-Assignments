...
Address Config::AddressMapping(uint64_t hex_addr) const {
    ...
    if(address_mapping.size() != 14)
        ...
        int lc = (hex_addr >> lc_pos) & lc_mask;
        int hc = (hex_addr >> hc_pos) & hc_mask;
        co = (hc << lc_pos) + lc;
    }
...
void Config::InitOtherParams() {
    ...
    if(address_mapping.size() == 12){
        output_prefix =
            output_dir + reader.Get("other", "output_prefix", "dramsim3") + "-" + row_buf_policy + "-" + mem_sch_policy + "-ROW";
    } else {
        output_prefix =
            output_dir + reader.Get("other", "output_prefix", "dramsim3") + "-" + row_buf_policy + "-" + mem_sch_policy + "-CACHE";
    }
...
void Config::InitSystemParams() {
    ...
    if (address_mapping.size() == 14){
        low_columns = GetInteger("system", "low_columns", 3);
        high_columns = GetInteger("system", "high_columns", 8);
    }
    queue_structure = reader.Get("system", "queue_structure", "PER_BANK");
    row_buf_policy = reader.Get("system", "row_buf_policy", "OPEN_PAGE");
    mem_sch_policy = reader.Get("system", "mem_sch_policy", "FRFCFS");
...
void Config::SetAddressMapping() {
    ...
    if(address_mapping.size() == 14) {
        field_widths["lc"] = LogBase2(low_columns);
        field_widths["hc"] = LogBase2(high_columns);
    }
    ...
    if(address_mapping.size() != 14){
        ...
        lc_pos = field_pos.at("lc");
        hc_pos = field_pos.at("hc");
        lc_mask = (1 << field_widths.at("lc")) - 1;
        hc_mask = (1 << field_widths.at("hc")) - 1;
    }
}
