...
Command CommandQueue::GetCommandToIssue() {
    for (int i = 0; i < num_queues_; i++) {
        ...
        if (config_.mem_sch_policy.compare("FRFCFS")){
            ...
            auto cmd = GetFirstComeFirstServe(queue);
            if (cmd.IsValid()) {
                if (cmd.IsReadWrite()) {
                    EraseRWCommand(cmd);
                }
...
Command CommandQueue::GetFirstComeFirstServe(CMDQueue& queue) const {
    if(queue.empty()) return Command();

    Command cmd = channel_state_.GetReadyCommand(*(queue.begin()), clk_);
