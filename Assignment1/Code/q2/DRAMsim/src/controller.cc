...
Controller::Controller(int channel, const Config &config, const Timing &timing)
    ...
    : channel_id_(channel),
      ...
      row_buf_policy_(config.row_buf_policy == "CLOSE_PAGE"
                          ...
                          : (config.row_buf_policy == "OPEN_PAGE" ? RowBufPolicy::OPEN_PAGE: RowBufPolicy::OPEN_4)),
      last_trans_clk_(0),
      write_draining_(0) {
...
Command Controller::TransToCommand(const Transaction &trans) {
    ...
    if (row_buf_policy_ == RowBufPolicy::OPEN_PAGE) {
        ...
        auto cmd = Command(CommandType::WRITE, addr, trans.addr);
        //int open_row = channel_state_.OpenRow(cmd.Rank(), cmd.Bankgroup(), cmd.Bank());
		//CMDQueue cmd_q = cmd_queue_.GetQueue(cmd.Rank(), cmd.Bankgroup(), cmd.Bank());
        // 
		/*
        for (auto pending_itr = cmd_q.begin(); pending_itr != cmd_q.end(); pending_itr++) {
